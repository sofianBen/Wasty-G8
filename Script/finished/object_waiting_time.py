...
#########   chercher les positions gps dans les données pour restreindre la zone de recherche


def object_waiting_time(pid_sub_category,padvert_state):

    # Donnees ciblees sur la categorie et l'etat souhaite
    df_u = df[(df['id_sub_category'] == pid_sub_category) & (df['object_state'] == padvert_state)]
    ...
    # Calcul de la frequence en nombre de jour (Exemple : Une chaise apparait tous les 20 jours)
    freq = time_slot/len(df_u) 
  
    # Prochaine apparition en heure par rapport a la frequence et le dernier depot
    # Interpretation : Dans la prochaine heure, il va apparaitre 0.4 objet
    next_hour = 1/(freq*24/int(last))
  
    # Prochaine apparition en jour par rapport a la frequence et le dernier depot
    # Interpretation : Dans le prochain jour, il va apparaitre 9.6 objets
    next_day = 1/(freq/int(last))
  
    # Prochaine apparition en semaine par rapport a la frequence et le dernier depot
    # Interpretation : Dans la prochaine semaine, il va apparaitre 67.2 objets
    next_week = 1/(freq/(7+int(last)))
  
    # Prochaine apparition en mois par rapport a la frequence et le dernier depot
    # Interpretation : Dans le prochain mois, il va apparaitre 268.8 objets
    next_month = 1/(freq/(28+int(last)))
  
    # Calcul des probabilites d'apparitions des objets dans les 7jours et dans les 28jours
